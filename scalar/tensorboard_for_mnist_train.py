...
epoch = 100
display_step = 10
logs_path = "../graphs/tensorboard_mnist_train"
...
with tf.name_scope("bias"):
    biases = {
        'bc1': tf.Variable(tf.random_normal([32]), name="bias1"),
        'bc2': tf.Variable(tf.random_normal([64]), name="bias2"),
        'bd1': tf.Variable(tf.random_normal([1024]), name="bias3"),
        'out': tf.Variable(tf.random_normal([10]), name="bias4")
    }
...
def model(x, dropout):
    with tf.name_scope("model"):
        layer1 = tf.reshape(x, shape=[-1, 28, 28, 1], name="reshape1")
        layer2 = tf.nn.relu(
